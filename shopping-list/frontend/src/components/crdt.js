...
export class CommunicationComponent {
    constructor(app) {
      ...
      this.longPolling = function () {
        ...
        for (var key in this.correspondingApp) {
            console.log("Key: " + key + " Value: " + this.correspondingApp[key])
          }
        var xhr = new XMLHttpRequest();
        ...
        xhr.onreadystatechange = (function () {
          ...
          if (xhr.readyState == XMLHttpRequest.DONE && xhr.status == 200) {
            ...
            console.log("ENTERED XHR READY STATE")
            for (var key in this.correspondingApp) {
              if (this.correspondingApp[key].name === obj.crdtName && key !== "key") {
                var newObj = this.correspondingApp[key].downstream(obj.operation);
                this.correspondingApp[key] = newObj;
                // console log every element of corresponding app
                for (var key in this.correspondingApp) {
                  console.log("Key: " + key + " Value: " + this.correspondingApp[key])}
                }
            }
      ...
      this.setCRDT = function (crdt) {
        ...
        Object.keys(this.correspondingApp).forEach(function (key, index) {
          console.log("#Set");
          if (this.correspondingApp[key].name === crdt.name) {
            this.correspondingApp[key] = crdt;
          }
        }, this);
      };
      ...
      this.setCorrespondingApp = function (app) {
        this.correspondingApp = app;
    };
    }
  ...
  //The Last-Writer-Wins-Register!
  export class OpLwwRegister {
    constructor(name, defaultValue = false, date = new Date().getTime()) {
      this.name = name;
