...
public class Server {
    ...
    public Server(String ipAddress, int portNumber) throws IOException {
        ...
        this.filepathPrefix = "backend/server3/";
...
/*                case SYNC:
                    ...
                    if(obj7.getClass() == ShoppingList.class) {
                        ...
                        // check if file exists
                        File file = new File("backend/server3/" + listId.toString() + ".ser");
                        if(!file.exists()) {
                            Database.writeToFile(list, "backend/server3/" + listId.toString() + ".ser" );
                            break;
                        }

                        // read list from database
                        Object listObj  = Database.readFromFile("backend/server3/"
                                + listId.toString() + ".ser");
                        ...
                        if (listObj.getClass() == ShoppingList.class) {
                            ...
                            // delete list from database if exists else create
                            Database.deleteFile("backend/server3/" + listId.toString() + ".ser" );

                            // store list in database
                            Database.writeToFile(shoppingList, "backend/server3/" + listId.toString() + ".ser" );
